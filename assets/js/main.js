...
import { cercaSquadre, caricaDettagli, caricaProssimiEventi } from "./api.js";
import { formRicerca, inputRicerca, sezioneRisultati, sezioneDettagli, btnIndietro, dettagliHeader, listaProssimi, listaUltimi, risultatiHeading, sezioneAppuntamenti, btnModalChiudi, modalOverlay, } from "./dom.js";
import { debounce, impostaSpinner, mostraErrore, caricaPreferiti, renderPreferiti, renderAppuntamenti, renderSquadre, renderDettagli, chiudiModal, } from "./ui.js";
// ===== STATO =====
...
export async function caricaAppuntamenti() {
    ...
    const risultati = await Promise.allSettled(lista.map((p) => caricaProssimiEventi(p.id)));
    const tuttiEventi = [];
    for (const r of risultati) {
        if (r.status === "fulfilled")
            tuttiEventi.push(...r.value);
    }
...
// ===== NAVIGAZIONE =====
export async function apriDettagli(squadra) {
