...
import { Squadra, Evento } from "./api.js";
import {
  spinner,
  boxErrore,
  sezionePreferiti,
...
// Voce salvata in localStorage per ogni preferito (il campo `sport` non serve nella card)
...
export function salvaPreferiti(lista: Preferito[]): void {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(lista));
}

export function ePreferito(id: string): boolean {
  return caricaPreferiti().some((p) => p.id === id);
}

export function aggiungiPreferito(squadra: Squadra): void {
...
export function rimuoviPreferito(id: string): void {
...
export function renderPreferiti(): void {
  const lista = caricaPreferiti();
...
export function renderAppuntamenti(eventi: Evento[]): void {
...
export function renderSquadre(squadre: Squadra[]): void {
  ...
  for (const squadra of squadre) {
    ...
    if (isPref) variante = "btn-secondary";
...
function creaVoceEvento(evento: Evento): HTMLLIElement {
  const li = make("li", {
...
function riempiLista(
  ...
  if (eventi.length === 0) {
    lista.append(
      make("li", { className: "text-muted fst-italic small", textContent: testoVuoto }),
    );
  } else {
...
export function renderDettagli(
  squadra: Squadra,
  prossimi: Evento[],
  ultimi: Evento[],
): void {
...
export function apriModal(evento: Evento): void {
  ...
  for (const [etichetta, valore] of righe) {
    const riga = make("div", {
      className:
        "d-flex justify-content-between align-items-baseline gap-3 py-2 small border-bottom modal-riga",
    });
    riga.append(
      make("span", {
...
export function chiudiModal(): void {
